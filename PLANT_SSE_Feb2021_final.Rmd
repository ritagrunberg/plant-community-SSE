...
#disease_me <- lmer((AUDPS_parasite) ~ Year * Treat + (1|Plot), plant_SSE)

#anova(disease_me)
#summary(disease_me)
#anova(disease_me)
#Anova(disease_me, type ='III')
...
library(afex)
aov_car((AUDPS_parasite) ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
```
...
aov_car((Schedonorus_arundinaceus) ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
(summary(sp_aov))
aov_car(log10(sp_rich) ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
#anova(sp_aov_me)
#require(multcomp)
#sp_tukey <-summary(glht(sp_aov_me, linfct=mcp(Treat = "Tukey")), test = adjusted(type = 'fdr'))
#tuk.cld <- cld(sp_tukey)   # letter-based display
#tuk.cld
...
aov_car(hill_shannon ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
```
```{r diversity figure}
#jpeg(filename="figure1_plantdiversity.jpeg", width=180, height=90, units="mm", bg="white", res=300)
ggarrange(richness, shannon, ncol=1, nrow=2, labels = 'AUTO')
#dev.off()
```
...
summary(simp_aov)
aov_car(hill_simpson ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
summary(bio_aov)
aov_car(log_biomass ~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
group_manova <- manova(cbind(Forbs,  Grasses, Woody) ~ Treat * Year + Error(Plot/Year), 
                            contrasts = list(Treat =contr.sum, 
                                             Year = contr.sum),
                            data= plant_SSE)
#(cbind(Forbs,  Grasses, Woody)~ Year * Treat + Error(Plot/(Year)), plant_SSE)
...
summary(cent.modrm2)
aov_car(log_dist ~ Treat * Year + Error(Plot/Year), data= drift_cent)
