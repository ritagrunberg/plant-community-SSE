...
plant95quant_widener <- plant_height_Widener2020 %>%
  group_by(Species) %>%
  summarise(Max.height.cm =quantile(Max.height.cm, c( .95))) %>% #get 95% quantile for max height 
  filter(Max.height.cm > 30)  # remove Poa from dataset, not comfortable with ID 
```
...
plant_SSE <- plant_SSE %>% mutate(forb_relative = Forbs/total_cover,
                                  ...
                                  grass_relative = C3_relative + C4_relative,
...
plant_SSE %>% 
  group_by(Year, Treat)%>% 
  mutate(mean = mean(native_relative), std = sd(native_relative)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y =native_relative, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
...
native_anova <- aov(native_relative ~ Year * Treat + Error(Plot/Year), plant_SSE)
...
plant_SSE %>% 
  group_by(Year, Treat)%>% 
  mutate(mean = mean(exotic_relative), std = sd(exotic_relative)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y =exotic_relative, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
...
exotic_anova <- aov(exotic_relative ~ Year * Treat + Error(Plot/Year), plant_SSE)
...
grass <-plant_SSE %>% 
  group_by(Year, Treat)%>% 
  mutate(mean = mean(grass_relative), std = sd(grass_relative)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y = grass_relative, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
