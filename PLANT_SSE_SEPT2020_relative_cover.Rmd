...
pcoordO <- read_csv("C:/Users/grunberg/Documents/GitHub/plant-community-SSE/Data/plantsp_coordinates_BC.csv")
...
plant_tidy %>% filter(Max_height_cm >0)%>%
 ...
 # filter(cover <76)%>%
  ...
  annotate("rect", xmin = 64, xmax = 120, ymin = -1, ymax = 2, alpha = 0.35, fill="grey")+
  ...
  labs(x="Max plant height (cm) data from Wilfahrt 2020 Supp",  y="log (% cover)")
...
#  geom_hline(yintercept = 65.8, lwd=1, lty=2)+
  labs(x="Change in cover 2017 to 2018", y= "Max plant height (cm) data from Wilfahrt 2020 Supp")+
...
pt2 <-plant_change %>% 
  ...
  # geom_hline(yintercept = 65.8, lwd=1, lty=2)+
  #  geom_hline(yintercept = 65.8, lwd=1, lty=2)+
  labs(x="Change in cover 2018 to 2019", y= "Max plant height (cm) data from Wilfahrt 2020 Supp")+
  theme_bw()+facet_wrap(~Treat)
...
plot<-plant_SSE[,c(1:5,110:111,118:120)]# makes new matrix with envi data (columns 1-3)
...
***VECTORS
                  ...
                  NMDS1    NMDS2     r2 Pr(>r)  
log_biomass     0.98973  0.14295 0.0405  0.018 *
AUDPS_parasite -0.98023 -0.19785 0.0145  0.238  
Anthracnose    -0.91150 -0.41129 0.0153  0.209  
Rhiz           -0.68872  0.72503 0.0024  0.795  
Rust           -0.78692 -0.61705 0.0219  0.102  
####################################################################################
Call:
...
#write.csv(pcoordO, 'plantsp_coordinates_BC.csv')
...
drift_cent %>%  mutate(Year = as.factor(Year))%>%
  ...
  # arrange(Year) %>% 
  geom_boxplot(aes(x=Year, y = log10(distance_centroid), colour=Treat), width=0.5, position = dodge, outlier.colour = NULL)+ 
  geom_boxplot(aes(x=Year, y = log10(distance_centroid), fill=Treat), width=0.5, position = dodge, outlier.size=-1)+ 
  rita_theme + 
  scale_fill_manual(values=c('#ffffcc','#a1dab4','#41b6c4','#225ea8'),
                    guide=guide_legend(override.aes=list(shape=21)), 
                    labels = c("Never", "Always", "CR->P", "CRP")) +
  ...
  guides(colour=FALSE)+
  annotate("text", x = 1, y = 0, label = "A", size=4) + #2017
  annotate("text", x = 2, y = 1, label = "B", size=4) + #2018
  annotate("text", x = 3, y = 1, label = "B", size=4) +  #2019
  # 
  annotate("text", x = 0.80, y = -0.55, label = "a", size=2) + #17 never sprayed
  annotate("text", x = 0.90, y = -0.55, label = "a", size=2) +  #17 always sprayed
  annotate("text", x = 1.05, y = -0.55, label = "a", size=2) +  #17 cr->p
  annotate("text", x = 1.20, y = -0.55, label = "a", size=2) +  #17 crp
  #
  annotate("text", x = 1.80, y = 0, label = "a", size=2) + #18 never sprayed
  annotate("text", x = 1.90, y = 0, label = "ab", size=2) +  #18 always sprayed
  annotate("text", x = 2.05, y = 0, label = "ab", size=2) +  #18 cr->p
...
for (i in plot_list){
  ...
  plot_moved<- data.frame(distance2017_2018 = dist.pt(as.numeric(plot_target[1,6]), as.numeric(plot_target[1,7]),
                                                       as.numeric(plot_target[2,6]), as.numeric(plot_target[2,7] )),
                           distance2018_2019 = dist.pt(as.numeric(plot_target[2,6]), as.numeric(plot_target[2,7]),
                                                       as.numeric(plot_target[3,6]), as.numeric(plot_target[3,7] )),
                          distance2017_2019 = dist.pt(as.numeric(plot_target[1,6]), as.numeric(plot_target[1,7]),
                                                       as.numeric(plot_target[3,6]), as.numeric(plot_target[3,7] ))
                          )
