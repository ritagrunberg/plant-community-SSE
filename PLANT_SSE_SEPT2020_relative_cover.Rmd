...
# library(mvabund)
# library(dplyr)
# plant_list <- plant_community_diversity %>% select(8:105) %>% select_if(colSums(.) != 0)
# 
# plant.data <- mvabund(plant_list)
# is.mvabund(plant.data)
# 
# #constrain the permutation within a plant by designating shuffletset; nrow = within the abundance data and control = plant/block 
# permID <- shuffleSet(n=nrow(plant.data), nset=99, control=how(block=plant_community_diversity$Plot))
# mod1 <- manyglm(plant.data ~ plant_community_diversity$Treat* plant_community_diversity$Year, family="poisson")
# plot(mod1)
...
plant_spp_syn <- plant_tidy %>%
  group_by(Plot, Treat, Species) %>%
  mutate(sd_spp = sd(cover), #stdev of a species across time within a plot 
         mean_spp = mean(cover), #mean of that species across time within a plot 
         cv_spp = sd_spp/mean_spp)%>% #coeff of variaiton of that species 
  ungroup()%>%
  group_by(Plot, Treat) %>%
  summarise(sd_all = sd(cover), 
         total_sd = sum(sd_spp),
         community_sync = sd_all/total_sd,
         mean_rich = mean(sp_rich))

plant_spp_syn %>% 
  ggplot()+
  geom_boxplot(aes(x=Treat, y = community_sync, fill=Treat))+
...
plant_spp_syn %>%
  ggplot()+
  geom_point(aes(x=mean_rich, y = community_sync, fill=Treat),pch=21, size=3)+
  scale_fill_manual(values=c('#fed976','#a1dab4','#41b6c4','#225ea8'),
                    guide=guide_legend(override.aes=list(shape=21)), 
                    labels = c("Never", "Always", "CR->P", "CRP"))+
  facet_wrap(~Treat)+
...
plant_species_syn <- plant_tidy %>%
  group_by(Plot, Treat, Species) %>%
  summarise(sd_spp = sd(cover), #stdev of a species across time within a plot 
         mean_spp = mean(cover), #mean of that species across time within a plot 
         cv_spp = sd_spp/mean_spp)%>% #coeff of variaiton of that species 
  ungroup()%>% drop_na()%>%
  group_by(Plot, Treat) %>%
  mutate(community_stability = mean(cv_spp))

plant_species_syn%>%
  ggplot()+
  geom_boxplot(aes(x = Treat, y = community_stability))+
  rita_theme
```
...
#disease_anova2 <- aov((AUDPS_parasite) ~ Year * Treat, plant_SSE)
...
library(DescTools)
EtaSq(disease_anova, type=1, anova=TRUE)
...
fes_emm2 <- emmeans(fes.md, ~ Year*Treat)
fescue_out <- data.frame(fes_emm2)
```
...
ggplot(fescue_out, aes(x= Year, y = emmean, group=Treat))+
  rita_theme+
...
```{r richness predict richness}
richrich <- plant_SSE %>%
  spread(Year, sp_rich)%>%
  replace(is.na(.), 0) %>%
  dplyr::select(Plot, Treat, '2017', '2018', '2019')%>%
  group_by(Plot, Treat)%>%
  summarise_all(sum)%>%
  rename(sp17 = '2017', sp18 = "2018", sp19 = "2019")

richrich %>%
  ggplot()+
  geom_point(aes(x=sp17, y = sp18, fill=Treat), pch=21, size=2.5)+ 
    scale_fill_manual(values=c('#ffffcc','#a1dab4','#41b6c4','#225ea8'),
                    guide=guide_legend(override.aes=list(shape=21)), 
                    labels = c("Never", "Always", "CR->P", "CRP")) +
  guides(fill=guide_legend("Treatment",  keywidth=0.12,
...
richrich %>%
  ggplot()+
  geom_point(aes(x=sp18, y = sp19, fill=Treat), pch=21, size=2.5)+ 
    scale_fill_manual(values=c('#ffffcc','#a1dab4','#41b6c4','#225ea8'),
                    guide=guide_legend(override.aes=list(shape=21)), 
                    labels = c("Never", "Always", "CR->P", "CRP")) +
  guides(fill=guide_legend("Treatment",  keywidth=0.12,
...
richrich %>%
  ggplot()+
  geom_point(aes(x=sp17, y = sp19, fill=Treat), pch=21, size=2.5)+ 
    scale_fill_manual(values=c('#ffffcc','#a1dab4','#41b6c4','#225ea8'),
                    guide=guide_legend(override.aes=list(shape=21)), 
                    labels = c("Never", "Always", "CR->P", "CRP")) +
  guides(fill=guide_legend("Treatment",  keywidth=0.12,
...
Sanity check to see if lmer gives same result as RM-ANOVA function... it does :) 
...
#tuk <- glht(rich_aov_me, linfct = mcp(Treat = "Tukey"))
...
ggplot(data=sprichout, aes(x=Year, y = 10^(emmean), group=Treat))+
  ...
  geom_point(aes(x=Year, y = (sp_rich), group=Treat, fill=Treat), pch=21, data=plant_SSE, 
             position=position_jitterdodge(dodge.width=0.70,  jitter.width = 0.3), 
             alpha=0.35, size = 1)+
 ...
 # ylim(c(1,14))+
  ...
  annotate("text", x = 1.25, y = 8.4, label = "ab", size=2) +  #17 crp
  ...
  annotate("text", x = 2.25, y = 10.7, label = "ab", size=2) +  #18 crp
  ...
  annotate("text", x = 3.25, y = 7.8, label = "ab", size=2)
#dev.off()
...
Looking at temporal heterogenity among treatments.... 

```{r temporal heterogenity}
plant_temporal <- plant_SSE %>%
  group_by(Plot, Treat)%>%
  summarise(rich_temporal = sd(sp_rich), 
            mean_rich = mean(sp_rich),
            rich_heter = rich_temporal/mean_rich,
            fescue_temporal = sd(Schedonorus_arundinaceus),
            mean_fescue = mean(Schedonorus_arundinaceus),
            fescue_heter = fescue_temporal/mean_fescue,
            biomass_temporal = sd(total_biomass),
            mean_biomass = mean(total_biomass),
            biomass_heter = biomass_temporal/mean_biomass,
            anth_temp = sd(Anthracnose),
            anth_mean = mean(Anthracnose),
            anth_heter = anth_temp/anth_mean,
            rust_temp = sd(Rust),
            rust_mean = mean(Rust),
            rust_heter = rust_temp/rust_mean,
            rhiz_temp = sd(Rhiz),
            rhiz_mean = mean(Rhiz),
            rhiz_heter = rhiz_temp/rhiz_mean)

plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = rich_heter))+theme_bw()

plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = fescue_heter))+theme_bw()

plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = biomass_heter))+theme_bw()

######## now for pathogens 
plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = anth_heter))+
  labs(x="",y="Anthracnose temporal heterogeneity")+
  theme_bw()

plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = rust_heter))+
  labs(x="",y="Rust temporal heterogeneity")+
  theme_bw()

plant_temporal %>%
  ggplot()+
  geom_boxplot(aes(x=Treat, y = rhiz_heter))+
  labs(x="",y="Rhizoctonia temporal heterogeneity")+
  theme_bw()

plant_temporal %>%
  ggplot()+
  geom_point(aes(x=rich_heter, y = biomass_heter))+
  facet_wrap(~Treat)+
  theme_bw()
#temp_rich <-aov(temporal_heterogenity~ Treat, data= plant_temporal)
#summary(temp_rich)
...
#jpeg(filename="biomass_trt.jpeg", width=90, height=90, units="mm", bg="white", res=300)
# biomass <- ggplot(data=biomass_plot, aes(x=Year, y = mean_bio, group=Treat))+
#   geom_point(aes(x=Year, y = log_biomass, group=Treat, fill=Treat), pch=21, data=biomass_plot, position = dodge, alpha=0.25)+
#   geom_errorbar(err_stdev, position=dodge, width=0.1) +
#   geom_point(aes(x=Year, y =mean_bio, fill=Treat), 
#              pch=21, size=2.5, position=dodge)+
...
#   annotate("text", x = 1, y = 3.35, label = "B", size=4) + #2017
#   annotate("text", x = 2, y = 3.35, label = "B", size=4) + #2018
#   annotate("text", x = 3, y = 3.5, label = "A", size=4) +  #2019
#   
#   annotate("text", x = 0.73, y = 3.1, label = "c", size=2) + #17 never sprayed
#   annotate("text", x = 0.9, y = 3.2, label = "a", size=2) +  #17 always sprayed
#   annotate("text", x = 1.10, y = 3.15, label = "b", size=2) +  #17 cr->p
#   annotate("text", x = 1.25, y = 3.2, label = "a", size=2) #+  #17 crp
#   
  # annotate("text", x = 1.73, y = 3.0, label = "c", size=2) + #18 never sprayed
  # annotate("text", x = 1.9, y = 3.22, label = "a", size=2) +  #18 always sprayed
  # annotate("text", x = 2.10, y = 3.15, label = "b", size=2) +  #18 cr->p
  # annotate("text", x = 2.25, y = 3.22, label = "a", size=2) +  #18 crp
  # 
  # annotate("text", x = 2.73, y = 3.3, label = "c", size=2) + #19 never sprayed
  # annotate("text", x = 2.9, y = 3.45, label = "a", size=2) +  #19 always sprayed
  # annotate("text", x = 3.05, y = 3.3, label = "b", size=2) +  #19 cr->p
...
#                            default.unit="inch"), color= FALSE) +
#   # ylim(c(1,14))+
#   annotate("text", x = 1, y = 9.5, label = "B", size=3.5) + #2017
#   annotate("text", x = 2, y = 13.5, label = "A", size=3.5) + #2018
#   annotate("text", x = 3, y = 10, label = "C", size=3.5) +  #2019
#   
#   annotate("text", x = 0.73, y = 8, label = "a", size=2) + #17 never sprayed
#   annotate("text", x = 0.9, y = 7.6, label = "b", size=2) +  #17 always sprayed
#   annotate("text", x = 1.10, y = 7.6, label = "b", size=2) +  #17 cr->p
#   annotate("text", x = 1.25, y = 7.7, label = "ab", size=2)# +  #17 crp
#   
#   # annotate("text", x = 1.73, y = 11.2, label = "a", size=2) + #18 never sprayed
#   # annotate("text", x = 1.9, y = 10.5, label = "b", size=2) +  #18 always sprayed
#   # annotate("text", x = 2.10, y = 10.5, label = "b", size=2) +  #18 cr->p
...
# library(cowplot)
# 
# jpeg(file="richness_biomass_SSE.jpeg",width = 90, height = 160,  units = 'mm',  res = 300) 
# plot_grid(richness, biomass, ncol=1, align = "v", rel_widths = c(1, 1.2))
# dev.off()
