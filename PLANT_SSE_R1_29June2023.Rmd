...
setwd("C:/Users/rlg59/Documents/GitHub/plant-community-SSE")

plant_groups <- read.csv("./Data/plant_groups.csv")
plant_community <- read.csv("./Data/plant_community_fixed_2Dec2019.csv",fileEncoding = 'UTF-8-BOM')
plot_treatment <- read.csv("./Data/plot_treatments.csv",fileEncoding = 'UTF-8-BOM')
plotbiomass_2017 <- read.csv("./Data/2017PlotBiomass_SSprayExp.csv", fileEncoding = 'UTF-8-BOM')
plotbiomass_2018 <- read.csv("./Data/2018PlotBiomass_SSprayExp.csv",  fileEncoding = 'UTF-8-BOM')
plotbiomass_2019 <- read.csv("./Data/2019PlotBiomass_SSprayExp.csv", fileEncoding = 'UTF-8-BOM')
SSE_prevalence_overall <- read.csv("./Data/SSE_prevalence_overall.csv", fileEncoding = 'UTF-8-BOM') #overall prevalence 
SSE_prevalence_spp <- read.csv("./Data/SSE_prevalence_spp.csv")
...
plant_community <-plant_community %>% select(-c(X))

#merge DFs
plant_community_duex <- merge(plot_treatment, plant_community, by=c('Plot')) 
plant_community_duex$total_cover <- rowSums(plant_community_duex[,c(8:103)]) # total cover includes litter 
...
relative_cover<-apply(plant_community_duex[,8:103],2, function(x) x/plant_community_duex$total_cover)
total <- rowSums(relative_cover[,1:96]) # checks that this works, so not all plots will add up to 100 because of the litter 
max(total)
min(total)
...
Treat <- c("Never", "Year-round", "7-months", "9-months")
...
To run fasterer can skip this part 
...
# 
# mod0 <- manyglm(plant.data ~ plant_community_diversity$Year* plant_community_diversity$Treat, family = "negative.binomial")
# sppcoef <- coef(mod0)
# #coefplot(mod0)
# plot(mod0)
# summary(mod0)
...
write.csv(biomass, 'plot_biomass_171819.csv')
...
#write.csv(biomass_wide, 'plot_biomass_sorted_2018.csv')
...
ggplot(data=sort_biomass_means, aes(x=Treat_order, y = mean, fill=Biomass.category))+
  ...
  annotate("text", x = 2.85, y = 1000, label = "ab", size=3) +  #fescue
...
plant_parasite <- plant_SSE %>% mutate(relative_anthracnose = Anthracnose/ AUDPS_parasite,
  ...
  gather(parasite, relative, 119:121) 
...
```
```{r analysis parasite composition}
  library(mvabund)
  disease_list <-  plant_SSE %>% mutate(relative_anthracnose = Anthracnose/ AUDPS_parasite,
                                       relative_rhiz = Rhiz/ AUDPS_parasite,
                                       relative_rust = Rust/AUDPS_parasite)%>%
    select(119:121)

disease.data <- mvabund(disease_list)
 is.mvabund(disease.data)
#
# #constrain the permutation within a plot by designating shuffletset; nrow = within the abundance data and control = plot/block
permID <- shuffleSet(n=nrow(plant_SSE),
                     nset=999,
                     control=how(block=plant_SSE$Plot))

mod0 <- manyglm(disease.data ~ plant_SSE$Year* plant_SSE$Treat, family = "negative.binomial")
sppcoef <- coef(mod0)
coefplot.manyglm(mod0, which.Xcoef=4:8)
plot(mod0)
summary(mod0)
set.seed(51)
mod.out <- anova(mod0,
...
mod.out
mod.out.posthoc <- anova(mod0,
                         pairwise.comp = as.factor(plant_SSE$Treat),
                         bootID=permID, 
                         p.uni = 'adjusted',
                         test = 'LR')
mod.out.posthoc2 <- anova(mod0,
                         pairwise.comp = as.factor(plant_SSE$Year),
                         bootID=permID, 
                         p.uni = 'adjusted',
                         test = 'LR')
mod.out.posthoc
...
Pt 1. b-c. Does fungicide impact plant Shannon and Simpson hill diversity 
