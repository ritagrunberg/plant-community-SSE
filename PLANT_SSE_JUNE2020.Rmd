...
forb1 <-plant_SSE %>%
  group_by(Year, Treat)%>% 
  mutate(mean = mean(Forbs), std = sd(Forbs)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y = Forbs, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
...
legumes1 <-plant_SSE %>% 
  group_by(Year, Treat)%>% 
  mutate(mean = mean(Legumes), std = sd(Legumes)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y = Legumes, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
...
##### WOODY 
###########################################################################################################
Woody_anova <- aov(Woody ~ Year *Treat + Error(Plot/Year), plant_SSE)
summary(Woody_anova)
# using car package 
...
woody1 <-plant_SSE %>% 
  group_by(Year, Treat)%>% 
  mutate(mean = mean(Woody), std = sd(Woody)) %>%
  ggplot(aes(x=Year, y = mean, group=Treat))+
  geom_point(aes(x=Year, y = Woody, group=Treat, fill=Treat), pch=21, position = dodge, alpha=0.25)+
  geom_errorbar(aes(ymin=mean - std, ymax=mean + std),position=dodge, width=0.1) +
  geom_point(aes(x=Year, y =mean, fill=Treat), 
             pch=21, size=2.75, position=dodge)+
  rita_theme+
